...
function init_map() {
	...
	var streams = new ol.layer.Image({
		source: new ol.source.ImageWMS({
			url: 'https://tethys2.byu.edu/geoserver/venezuela_hydroviewer/wms',
			params: { 'LAYERS': 'south_america-venezuela-geoglows-drainage_line' },
	...
	var stations = new ol.layer.Image({
		source: new ol.source.ImageWMS({
			url: 'https://tethys2.byu.edu/geoserver/venezuela_hydroviewer/wms',
			params: { 'LAYERS': 'Estaciones_Hidrometricas_Venezuela' },
			serverType: 'geoserver',
			crossOrigin: 'Anonymous'
		})
	});
	...
	map = new ol.Map({
		...
		view: new ol.View({
			center: ol.proj.fromLonLat([-66.933333, 10.5]),
			zoom: 5
		})
	});
...
let ajax_url = 'https://tethys2.byu.edu/geoserver/venezuela_hydroviewer/wfs?request=GetCapabilities';
...
let capabilities = $.ajax(ajax_url, {
	...
	success: function() {
		...
		.split('venezuela_hydroviewer:south_america-venezuela-geoglows-drainage_line')[1]
				...
				$.ajax({
					...
					success: function (result) {
						watershed = 'south_america' //OJO buscar como hacerla generica
		         		...
		         		var startdate = '';
		         		stationcode = result["features"][0]["properties"]["SERIAL_EST"];
		         		stationname = result["features"][0]["properties"]["NOMBRE_EST"];
		         		streamcomid = result["features"][0]["properties"]["new_COMID"];
		         		stream = result["features"][0]["properties"]["NOMBRE_RIO"];
